...
import { getNoteName, getFullNoteOnFret } from '../services/musicService';
...
const Fretboard: React.FC<FretboardProps> = ({ tuning, fretCount, notesToHighlight, rootNote, viewMode, onNoteSelect }) => {
        ...
        <div className="select-none font-mono min-w-max">
                ...
                {tuning.map((stringNote, stringIndex) => (
                    <div key={stringIndex} className="flex items-center">
                        {frets.map((fret) => {
                            ...
                            const noteToDisplay = isIdentifierMode ? noteWithOctave : noteName;
                            ...
                            return (
                                <div 
                                    key={fret} 
                                    className={`relative w-20 h-10 flex items-center justify-center border-r-2 border-gray-600 bg-gradient-to-b from-gray-700 to-gray-800 ${isIdentifierMode ? 'cursor-pointer' : ''}`}
                                    onClick={handleFretClick}
                                >
                                    ...
                                    {stringIndex === Math.floor(tuning.length / 2) && fret === 12 && (
            ...
            <div className="flex">
                {frets.map(fret => (
                    <div 
                        key={fret} 
                        className={`${fret === 0 ? 'w-10' : 'w-20'} text-center text-xs text-gray-400 pt-2`}
                    >
                        {fret}
                    </div>
                ))}
